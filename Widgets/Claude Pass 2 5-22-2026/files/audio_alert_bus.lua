function widget:GetInfo()
  return {
    ...
    desc    = "Serializes voice/UI audio clips so they never overlap. Provides a global master volume slider for all queued alerts.",
...
options = {
  voiceAlertMasterVolume = {
    name  = "Voice Alerts Master Volume",
    desc  = "Global volume multiplier applied to all queued voice alerts (0.10× – 5.00×).",
    type  = "number",
...
function widget:GetConfigData()
  return {
    voiceAlertMasterVolume = options.voiceAlertMasterVolume.value,
  }
end

function widget:SetConfigData(data)
  if not data then return end
  if data.voiceAlertMasterVolume then
...
-- Queue-only: nothing is ever dropped
local function enqueue(path, opts)
  ...
  -- Stable insert by priority (highest priority plays first)
