...
-- Player-visible system-style message (same style as pause/unpause toasts)
...
local function AssignLetterGroup(letter)
    ...
    if #selUnits == 0 then
        ...
        PlayerToast("No units selected to assign group " .. letter)
        return
    end
...
function widget:KeyPress(key, mods, isRepeat)
    if not options[1].value then return false end  -- ✅ Epic Menu gating
...
--------------------------------------------------------------------------------
-- MENU6 Additions: Settings label and A–Z Assign/Clear buttons
-- (Appended without altering MENU1's core behavior)
--------------------------------------------------------------------------------
do
    local labelOpt = {
        key   = "lbl_usage",
        name  = "Meta+letter selects • Shift adds • Ctrl filters • Backspace+Meta+letter clears",
        desc  = "",
        type  = "text",
        value = "",
    }

    -- Ensure 'options' table exists; insert label at top
    options = options or {}
    table.insert(options, 1, labelOpt)

    -- Append section headers and buttons
    local function _append(opt) options[#options+1] = opt end

    _append({ key = "lbl_assign_letters", type = "text",
              name = "Quick Assign (Capture Current Selection’s Unit Types)", value = "" })
    ...
    _append({ key = "lbl_clear_letters", type = "text",
              name = "Quick Clear (Erase Letter Group)", value = "" })
