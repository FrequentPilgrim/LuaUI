function widget:GetInfo()
  return {
    ...
    desc    = "Serializes voice/UI clips so they never overlap; global Master Volume slider up to 5×",
...
local function enqueue(path, opts)
  ...
  -- Debug line:
  -- Spring.Echo("BusEnqueue: " .. path .. " dur=" .. dur)
  return true
end
...
local function startNext()
  ...
  -- Debug line:
  -- Spring.Echo("BusPlay: " .. Bus.current.path)
end
...
function widget:Update()
  ...
  if DiffTimers(Bus.tNow, Bus.tStart) >= (Bus.current.duration or Bus.fallback) then
    -- Debug line:
    -- Spring.Echo("BusFinish: " .. Bus.current.path)
    Bus.playing = false
    Bus.current = nil
    startNext()
  end
