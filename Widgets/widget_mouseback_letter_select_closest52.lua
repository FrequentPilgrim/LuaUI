...
local sparklePhase = 0       -- rotation/twinkle phase
local huePhase     = 0       -- hue 0..1
local ARMS         = 12      -- increased spokes for the sparkle
...
-- Sparkle size slider (pixels) — default 40px, up to 150px
if not options.sparkleSize then
    options.sparkleSize = {
        name   = "Sparkle Size (px)",
        type   = "number",
        min    = 6,
        max    = 150,   -- increased upper bound
        step   = 1,
        value  = 40,    -- new default size
        path   = OPT_PATH,
...
function widget:Update(dt)
    ...
    if mouseBackHeld and options.sparkleIndicator and options.sparkleIndicator.value then
        -- Faster spin and hue cycle
        sparklePhase = (sparklePhase + dt * 12.0) % (2 * math.pi) -- was 6.0
        huePhase     = (huePhase + dt * 0.35) % 1.0               -- was 0.15
    end
end
...
function widget:DrawScreen()
    ...
    local arms  = ARMS            -- increased spokes
    local alpha = 0.6 + 0.35 * math.sin(sparklePhase * 3.5) -- faster flicker (was *2.2)
