function widget:GetInfo()
  return {
    name    = "Voice Alert Bus",
    desc    = "Serializes voice/UI clips so they never overlap; global Master Volume slider up to 5×. Supports immediate=true for reliable end-game announcements while still applying masterGain.",
...
-- This widget owns the complete ordering for Settings/Audio/Audio Alerts
--------------------------------------------------------------------------------

options_path = "Settings/Audio/Audio Alerts"

-- Simple ordering: ensure Master Volume appears (we can improve the full list later).
...
local function oggVorbisDurationSeconds(path)
  ...
  local sampleRate = le32(data, p + 11)
...
-- Queue-only behavior: nothing is ever dropped
local function enqueue(path, opts)
  ...
  local item = {
    ...
    channel  = (opts.channel ~= nil) and opts.channel or Bus.defaultChannel,
    duration = dur,
  ...
  -- Support for immediate high-priority playback.
  -- Used by audio_alert_victory_defeat_draw.lua for the mission end-screen
  -- ("Victory/Defeat + Click to continue") so the sound is delivered reliably
  -- even during bonus objective resolution / fade / pause states, while still
  -- respecting the user's Voice Alerts Master Volume slider.
  if opts.immediate then
    Bus.queue = {}                  -- preempt anything else
    Bus.current = item
    Bus.playing = true
    Bus.tStart  = GetTimer()
    local vol = (item.volume or 1.0) * (Bus.masterGain or 1.0)
    PlaySoundFile(item.path, vol, nil, nil, nil, nil, nil, nil, item.channel)
    if item.key then Bus.lastByKey[item.key] = Bus.tStart end
    return true
  end

  -- Stable insert by priority (normal queued behavior)
